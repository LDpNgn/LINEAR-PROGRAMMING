...
class LinearProgramming:
    ...
    def printSolution(self, step_by_step = False):
        if self.type_result == 'VSN':
            if step_by_step == True:
                st.write('Bien o ham muc tieu bi khuyet so voi cac rang buoc')
            st.write('Bai toan co vo so nghiem')
            st.write('Voi bo nghiem la: ')
            ...
            for x in self.x:
                st.write(x)
            st.write(f"Gia tri toi uu: {self.optimize_direction} z = {round(self.z, 2)}")
            return
    ...
    def call_result(self, df_result, step_by_step = True, method = 'bland'):
        ...
        st.write('type result: ', self.type_result)
